...
const PastAppointmentsPatient: React.FC<Props> = ({
  showAllPast,
  setShowAllPast,
  handleToggleShowAllAppointments,
}): JSX.Element => {
  ...
  const { myAppointments } = ptAppointments;
  ...
  const handleFirstSort = () => {
    ...
    const filteredPastApp = filterPastAppointments(
      myAppointments.pastAppointments
    );
    const sortedApp = sortByDatePastApp(filteredPastApp);
  ...
  const handleApplyFilter = (isDescending?) => {
    ...
    if (thirdQuery.length > 0) {
      ...
      const filteredPastApp = filterUpcomingAppointments(myAppointments);
  ...
  const handleSearchApp = (txt) => {
    ...
    let newApp;
    try {
      newApp = sortedAppointments.filter((app) => {
        let fullDocName =
          app.doctorInfo.doctorInfoData.firstname +
          " " +
          app.doctorInfo.doctorInfoData.lastname;
        return fullDocName.toLowerCase().includes(txt.toLowerCase());
      });
  ...
  return (
    <>
      ...
      <View style={styles.container}>
        ...
        <AppointmentFiltersPatient
          handleResetSort={handleFirstSort}
        ...
        >
          {showAllPast ? (
            ...
            >
              <H14fontRegularGray>
                Don't show all past appointments
              </H14fontRegularGray>
            </TouchableOpacity>
          ) : (
            <TouchableOpacity
              onPress={() => {
                handleToggleShowAllAppointments(false, true);
                setShowAllPast(!showAllPast);
              }}
            >
              <H14fontRegularGray>
        ...
        {searchedApp.length > 0 ? (
          ...
          <H6fontRegularBlue>
            {i18n.translate("noAppointemntsForPatient")}
          </H6fontRegularBlue>
        )}
      </View>
    </>
...
export default PastAppointmentsPatient;
